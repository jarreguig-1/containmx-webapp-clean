...
const priceList: Record<Medida, Record<string, Price>> = {
  20: {
    S1: { Plegable: 1848, Desmontable: 1698, nota: "1 puerta frontal" },
    S2: { Plegable: 2048, Desmontable: 1898, nota: "2 puertas frontales" },
    S3: { Plegable: 1848, Desmontable: 1698, nota: "1 puerta lateral" },
    S4: { Plegable: 2048, Desmontable: 1898, nota: "2 puertas laterales" },
    S5: { Plegable: 2148, Desmontable: 1998, nota: "3 puertas laterales" },
    S6: { Plegable: 2248, Desmontable: 2098, nota: "4 puertas laterales" },
    S7: { Plegable: 2248, nota: "4 puertas (2 por lateral)" },
    S8: { Plegable: 2708, nota: "8 puertas (4 por lateral)" },
    S9: { "Oficina/Recepcion": 3358 },
  },
  16: {
    S1: { Plegable: 1748, Desmontable: 1598, nota: "1 puerta frontal" },
    S2: { Plegable: 1958, Desmontable: 1808, nota: "2 puertas frontales" },
    S3: { Plegable: 1748, Desmontable: 1598, nota: "1 puerta lateral" },
    S4: { Plegable: 1958, Desmontable: 1808, nota: "2 puertas laterales" },
    S5: { Plegable: 2048, Desmontable: 1898, nota: "3 puertas laterales" },
  },
  12: {
    S1: { Plegable: 1457, Desmontable: 1457, nota: "1 puerta frontal" },
  },
  10: {
    S1: { Plegable: 1357, Desmontable: 1357, nota: "1 puerta frontal" },
  },
  8: {
    S1: { Plegable: 1257, Desmontable: 1257, nota: "1 puerta frontal" },
  },
...
const containerCapacity: Partial<Record<Medida, Partial<Record<Tipo, Partial<Record<string, number>>>>>> = {
  20: {
    Plegable: { S1: 14, S2: 14, S3: 14, S4: 14, S5: 13, S6: 13, S7: 13, S8: 12 },
    Desmontable: { S1: 13, S2: 12, S3: 13, S4: 12, S5: 11, S6: 11 },
    "Oficina/Recepcion": { S9: 10 },
  },
  16: {
    Plegable: { S1: 14, S2: 14, S3: 14, S4: 14, S5: 13 },
    Desmontable: { S1: 13, S2: 12, S3: 13, S4: 12, S5: 11 },
  },
  12: { Plegable: { S1: 18 }, Desmontable: { S1: 18 } },
  10: { Plegable: { S1: 20 }, Desmontable: { S1: 24 } },
      ...
      .map((p, idx) => {
        ...
        return `
          <tr>
            ...
            <td class="concepto">${escapeHtml(p.concept || "")}</td>
                          ...
                          {(rows || []).map((r, lineIdx) => {
                            ...
                            return (
                              <div key={`${idx}-${lineIdx}`} style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                                <span>
                                  {r.medida}' · {r.modelo} · {r.tipo} — {units} u × {fmtUSD(r.precioUnidad, false)}
                                </span>
...
export default function ContainMX() {
  ...
  // A partir de aquí ya existe `current` y `s` debería existir.
  ...
  const exportCotizacionPdf = () => {
    ...
    const rowsHtml = rows
      .map((r) => {
        ...
        const desc = `${r.modelo} ${r.tipo}${note ? ` · ${note}` : ""}`;
    ...
    const subTotal = round2(rows.reduce((acc, r) => acc + r.precioLinea, 0));
    const ivaTotal = round2(subTotal * IVA_RATE);
    const totalFinal = round2(subTotal + ivaTotal);
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .bank { margin-top: auto; }
            .signatures { margin-top: 36mm; }
            .sig-line { border-top: 1px solid #111; padding-top: 8px; min-height: 26mm; }
